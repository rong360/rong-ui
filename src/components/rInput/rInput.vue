  ...
  <div :class="['r-input', !conf.showBottomBorder?'r-border--off':'']">
    <div class="r-txt-wrap">
      <label class="r-input-label"
             :style="lblStyle"
             v-show="conf.showLabel">
        <span>{{conf.title}}</span>
        <!--
				-->
        <rIcon :type="conf.labelIconType"
               size="0.8rem"
               v-if="conf.showLabelIcon"
               @iconClick="onclickLabelIcon"></rIcon>
      </label>
      <div class="input-wrap">
        <input :type="conf.inputType"
               ...
               :class="iClsName">
        <rIcon :type="conf.inputIconType"
               size="0.8rem"
               v-show="showInputIcon"
               @iconClick="onclickInputIcon"></rIcon>
      </div>
      <slot>
        <div class="r-input-unit"
             v-if="conf.unit">{{conf.unit}}</div>
      </slot>
    </div>
  </div>
</template>
